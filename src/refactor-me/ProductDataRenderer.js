...
ProductDataRenderer.prototype.render = function() {
    ...
    var productSubsets = ProductDataConsolidator.get();

    for (var i = 0; i < productSubsets.length; i++) {
        var productSubset = productSubsets[i];

        //get product object literal
        var product = productSubset["product"];
        for (var j = 0; j < productSubsets.length; j++) {
            nzd +=
                    '<tr>'
                    + '<td>' + product[j].name + '</td>'
                    + '<td>' + product[j].price.toFixed(2) + '</td>'
                    + '<td>' + productSubsets[i].type + '</td>'
                    + '</tr>';
        }
    }
    nzd += '</tbody></table>';
